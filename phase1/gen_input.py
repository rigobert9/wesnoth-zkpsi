...
def write_input(filename, expfilename, prev_state, prev_misc_state, actions, degats,
    ...
    phase1_exponents = [impairer(num2bits(random.randint(0,modulo_courbe - 1))) for _ in range(state_size)]
    input_dict = {
            "prev_state": prev_state,
            "prev_misc_state": prev_misc_state,
            "degats": degats,
            "captures": captures,
            "phase1_exponents": phase1_exponents,
    }

    input_json = json.dumps(input_dict)
    exp_json = json.dumps(phase1_exponents)
